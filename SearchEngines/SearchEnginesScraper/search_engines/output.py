...
def create_csv_data(search_engines):
    ...
    data = [['Title', 'Content', 'EventTime', 'Domain', 'Url', 'Keyword', 'Engine']]
    for engine in search_engines:
        for i in engine.results:
            try:
                orgdate = find_date(i['link'])
                afterSplit = orgdate.split("-")
                eventTime = datetime(int(afterSplit[-3]), int(afterSplit[-2]), int(afterSplit[-1])).strftime("%Y-%m-%dT%H:%M:%SZ")
                print(str('from link: ') + eventTime)
            except:
                eventTime = None
                print(str('Url timeout'))
            row = [
                i['title'], i['text'], eventTime, i['host'], i['link'], engine._query, engine.__class__.__name__
            ]
